...
# 1. Detectamos dónde está el dashboard (carpeta 'dashboard')
DASHBOARD_PATH = os.path.dirname(os.path.abspath(__file__))

# 2. Subimos un nivel para llegar a la raíz del proyecto (HPE-CDS-Los-Foraneos)
PROJECT_ROOT = os.path.dirname(DASHBOARD_PATH)

# 3. Apuntamos a la carpeta exacta donde Webots (el Agente Edge) guarda los archivos
EDGE_CONTROLLER_PATH = os.path.join(PROJECT_ROOT, "controllers", "agente_edge_controller")

# 4. Asignamos los archivos a esa ruta compartida
FILE_CMD = os.path.join(EDGE_CONTROLLER_PATH, "comandos.json")
FILE_TELEMETRIA = os.path.join(EDGE_CONTROLLER_PATH, "telemetria.json")
FILE_FRAME = os.path.join(EDGE_CONTROLLER_PATH, "last_frame.jpg")
st.set_page_config(page_title="HPE Digital Twin - Predictive NOC", layout="wide")
RUTA_LOGO = os.path.join(DASHBOARD_PATH, "logo_principal.png")
